...
def confirm_entity(api, client, entity_name: str, exclusions: list) -> str:
 ...
 were found."
    ...
    expected_entity_name = input(
        f"\nEnter the name of the selected {entity_name}: "
    ).lower()
    matches = [
...
with ext_id {container_ext_id}.\n'
 ...
 {container_ext_id}."
    ...
    # the Cloud-Init script is only relevant for Rocky Linux 10 Cloud images
    print(
        "The Cloud-Init script used by this demo has been created for \
Rocky Linux 10 Cloud images with Cloud-Init pre-installed.  To use your own \
Cloud-Init userdata, modify userdata.yaml."
    ...
    new_vm = AhvVmConfig.Vm.Vm(
        ...
        boot_config = AhvVmConfig.UefiBoot.UefiBoot(
            boot_order = [
                AhvVmConfig.BootDeviceType.BootDeviceType.DISK
            ],
...
ext_id {new_vm_ext_id}.\n"
    ...
    power_on_confirm = utils.confirm("Power on VM now?")
    if power_on_confirm:
        existing_vm = vmm_instance.get_vm_by_id(new_vm_ext_id)
        # get the new VM's Etag
        existing_vm_etag = vmm_client.get_etag(existing_vm)
        existing_vm_ip = existing_vm.data.nics[0].network_info.ipv4_config.ip_address.value
        existing_vm_name = existing_vm.data.name
        # add the new VM's Etag as the value of If-Match header on the API client
        vmm_client.add_default_header(header_name="If-Match", header_value=existing_vm_etag)
        # recreate the VmApi instance now that we've added the Etag header to the API client instance
        vmm_instance = ntnx_vmm_py_client.api.VmApi(api_client=vmm_client)
        # power on the VM
        power_on = vmm_instance.power_on_vm(new_vm_ext_id)
        # get the power on task's details

        task_extid = power_on.data.ext_id
        utils.monitor_task(
            task_ext_id=task_extid,
            task_name="VM power on",
            pc_ip=script_config.pc_ip,
            username=script_config.pc_username,
            password=script_config.pc_password,
            prefix="",
        )
        print(f"\nVM name: {existing_vm_name}")
        print(f"VM IP: {existing_vm_ip}")
        print("VM power on action completed.\n")
    else:
        print("VM power on action cancelled. Exiting ...\n")
